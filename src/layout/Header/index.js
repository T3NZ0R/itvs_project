...
const Header = () => {
  const { routesDropdown, routesMobile } = useSelector(
      ...
      <div className={styles.headerWrapper}>
        <div className={styles.headerWebWrapper}>
          ...
          <div
            ...
            <div
              className={`${open && styles.headerHamburgerMenuLineTopOpened} ${
                styles.headerHamburgerMenuLineTop
              }`}></div>
            <div
              className={`${
                open && styles.headerHamburgerMenuLineCenterOpened
              } ${styles.headerHamburgerMenuLineCenter}`}></div>
            <div
              className={`${
                open && styles.headerHamburgerMenuLineBottomOpened
              } ${styles.headerHamburgerMenuLineBottom}`}></div>
          </div>
          <div className={styles.headerLinksWrapper}>
            <div className={styles.headerPagesLinks}>
              <Link href={'/'} className={styles.headerPageLink}>
                Головна
              </Link>
                ...
                <div className={styles.headerDropdownContent}>
                  {routesDropdown.map((route, index) => (
                    <React.Fragment key={route.title}>
                      <Link
                        className={styles.headerDropdownContentLink}
                        href={{
                          pathname: '/about-us',
              ...
              <Link href={'/events-and-news'} className={styles.headerPageLink}>
                Події та новини{' '}
              </Link>
              <Link
                href={'/department-staff'}
                className={styles.headerPageLink}>
                Колектив
              </Link>
              <Link href={'/contact-us'} className={styles.headerPageLink}>
                Контакти
              </Link>
            </div>
        ...
        {startClosed && (
          <div
            className={
              open && screenSize < 900
                ? styles.headerHamburgerMenuContentOpened
                : styles.headerHamburgerMenuContentClosed
            }>
            {routesMobile.map((route) => (
              <React.Fragment key={route.title}>
                {route.query ? (
                  <Link
                    className={styles.headerHamburgerMenuContentLink}
                    href={{ pathname: route.path, query: { tab: route.query } }}
                    onClick={() => {
                      handleOpen();
                    }}>
                    {route.title}
